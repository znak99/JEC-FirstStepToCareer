        ...
        VStack(spacing: proxy.size.height / 60) {
            ...
            HStack(spacing: proxy.size.width / 48) {
                ...
                ZStack {
                    ...
                    if !homeManager.currentCompanyName.isEmpty {
                        HStack {
                            Spacer()
                            Button(action: homeManager.clearCompanyNameField, label: {
            ...
            VStack(spacing: proxy.size.height / 80) {
                ...
                HStack {
                    Spacer()
                    Button(action: {
                        homeManager.isSaveInterviewOptions.toggle()
                    }, label: {
                        HStack(alignment: .center, spacing: proxy.size.width / 120) {
                            Image(homeManager.isSaveInterviewOptions ? "CheckBox-On" : "CheckBox-Off")
                                .resizable()
                                .frame(width: proxy.size.width / 20)
                                .frame(height: proxy.size.width / 20)
                            Text("この情報を保存する")
                ...
                if homeManager.isShowPermissionCaution {
                    Text("* 拒否された権限があります\n必要な権限「Speech, Camera, Microphone」")
                        .font(.custom(Font.customMedium, size: proxy.size.width / 32))
                        .foregroundStyle(.appRed)
                        .frame(maxWidth: .infinity)
                }
                ...
                Button(action: {
                    homeManager.submitInterviewInfo()
                }, label: {
                    Text("模擬面接を始める")
